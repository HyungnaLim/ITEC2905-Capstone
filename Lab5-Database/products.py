import sqlite3
from statistics import quantiles

# validation errors, row id

db = 'products.sqlite'

with sqlite3.connect(db) as conn :
    conn.execute('CREATE TABLE IF NOT EXISTS products (name TEXT UNIQUE, quantity INT)')
    # another version to create primary key
    # conn.execute('CREATE TABLE IF NOT EXISTS products (product_id INTEGER PRIMARY KEY,name TEXT UNIQUE, quantity INT)')
conn.close()

name = 'hat'
quantity = 10

# exception handling
try:
    with sqlite3.connect(db) as conn :
        conn.execute('INSERT INTO products VALUES (?, ?)', (name, quantity))
        # another version to create primary key
        # conn.execute('INSERT INTO products (name, quantity) VALUES (?, ?)', (name, quantity))
    conn.close()
except Exception as e :
    print('Error inserting:', e)


conn = sqlite3.connect(db)
results = conn.execute('SELECT rowid, * FROM products')
# sql makes row id which is auto increment number for each row
# easy way to create primary key!

# below will print with primary key if it is created with another version
# results = conn.execute('SELECT * FROM products')

for row in results:
    print(row)


print('End of program!')